...
-- 需要Ace-who的原代码
-- 
-- local t = require("xuma_precand")
-- xuma_precand = t.filter
-- xuma_precand_processor = t.processor
...
local options = {
  'xuma_precand.off',
  'xuma_precand.brief',
  'xuma_precand.detailed'
}
options.default = 2


local processor = rime.make_option_cycler(options,
    'xuma_precand/lua/cycle_key',
    'xuma_precand/lua/switch_key')
...
local function get_seg_cands(input, detailed, env)
  ...
  if input:len() < 5 then
    ...
    local codes = { input .. '_1', input .. '_2', input .. '_3', input .. '_4', input .. '_5',}
    return map(codes, lookup(env.code_to_text_rvdb))
  else
    local seg1 = input:sub(1, 2, 4, 5)
    local seg2 = input:sub(5)
    cand1 = env.code_to_text_rvdb:lookup(seg1 .. '_1')
    local codes = { seg2 .. '_1', seg2 .. '_2', seg2 .. '_3', seg2 .. '_4', seg2 .. '_5' }
...
local function yield_with_precand(input, context, env)
  ...
  local detailed = context:get_option("xuma_precand.detailed")
  -- 这是以候选迭代为基础的，因此要求无空码。
...
local function filter(input, env)
  ...
  if not context:get_option("xuma_precand.off")
